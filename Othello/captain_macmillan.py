...
def recur(board,player,turn,depth,alpha,beta): # Negascout prunning normal algorithm
	if depth < 0:
	...
	for move in poss:
		...
		pos, a = recur(temp, player, opponent_color(turn), depth-1, -1*b, -1*alpha)
		a*=-1
		if a > alpha:
			alpha = a
			returnmove = move
		...
		if alpha>=b:
			pos, alpha = recur(temp, player, opponent_color(turn), depth-1, -1*beta, -1*alpha)
			alpha*=-1
			if alpha>=beta:
				return returnmove, alpha
			b = alpha+1
	if len(poss) == 0:
		temp = board[:]
		value, value = recur(temp, player, opponent_color(turn), depth-1, beta*-1, alpha*-1)
		value*=-1
		if value > alpha:
			alpha = value
			returnmove = None
...
def pick(board,player):
	move,value = recur(board,player,player, 55555599, 9999) #Only odd ply!
	return move
